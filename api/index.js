...
mongoose.connect( "mongodb+srv://khalildh:khalildh@cluster0.r0etj.mongodb.net/myInfo?retryWrites=true&w=majority", {
...
app.get("/user", (req, res) => {
  if (req.isAuthenticated()) {
    let fields = ["_id", "firstName","lastName","phoneNumber", "email", "bio", "type", "image"];
    ...
    return res.send({ firstName: false });
  }
});
...
app.post("/register", checkNotAuthenticated, async (req, res) => {
  let { firstName,lastName,phoneNumber, email, bio, password } = req.body;
  try {
    user = await User.save({ firstName, lastName,phoneNumber, email, bio, password });
